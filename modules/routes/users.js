exports.users = (app, client, database) => {
    ...
    app.get('/login', async (req, res) => {
        try {
            ...
            if (comparePassword === true) {
                const accessToken = jwt.sign({ email: req.headers['email'], role: req.headers.role }, process.env.JWT_SECRET, { expiresIn: '1h' });
                const refreshToken = jwt.sign({ email: req.headers['email'], role: req.headers.role }, process.env.JWT_SECRET, { expiresIn: '1d' });
            ...
            if (req.user.role == "admin") {
                ...
                if ( checkUser.length === 0 ) {
                    ...
                    if ( userData.email && userData.password && userData.role ) {
                        ...
                        const result = await collection.insertOne({ email: userData.email, password: userData.password, role: userData.role, name: userData.name, surname: userData.surname, usage: userData.usage });
    ...
    app.put('/user', authenticateToken, async (req, res) => {
        ...
        try {
            ...
            if (req.body.name && req.body.surname) {
                ...
                const checkUser = await collection.find({email: req.user.email}).toArray();
                
                if ( checkUser.length != 0 ) {

                    const result = await collection.updateOne({ email: req.user.email }, { $set: { name: req.body.name, surname: req.body.surname } });
    ...
    app.get('/refresh', expiredToken, async (req, res) => {
        ...
        try {
                ...
                const existingRefreshToken = await database.collection('refresh').findOne({ refreshToken: refreshToken });
